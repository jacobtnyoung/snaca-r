...
Following @breiger1974duality, we can build the adjacency matrix for each projected network through matrix algebra. Specifically, multiplying an adjacency matrix by its **transpose**. The transpose of a matrix *A* simply reverses the columns and rows: $\sf{A^T_{ij}}$ = $\sf{A_{ji}}$.

The two-mode, *NxM*, adjacency matrix, when multiplied by its transpose, produces either:
...
To create the project, we need to use matrix algebra. To multiply matrices in R, we have to use the following operator: `%*%`. This is different from `*` in that `%*%` tells R to use matrix multiplication. For example, compare the differences:
...
To visualize this by plotting each network, we can better see what is happening. We will use the `gplot()` function, so be sure to call the `sna` package library.
...
This network is available in `SNACpack` under the name `officer_event_net`. We can type the name of the network to see its properties:
