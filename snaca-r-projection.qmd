...
Ok, so we have created the projections, which are the one-mode networks that represent information in the two-mode networks. Recall from the [Projection]() chapter that @breiger1974duality referred to the *person* matrix (i.e. the *NxN* matrix) and the *group* matrix (i.e. the *MxM* matrix).
