...
As discussed in [@wasserman1994social, p. 200-201], the denominator is not defined for closeness centralization for directed graphs. Yet, the score can be computed using the `centralization()` function.
...
- The *get along with network* was created by asking individuals with whom they "get along" on the unit. We can think of this as "friends" in a prison setting. (People don't really have "friends" in prison, but there are people they "get along with")

- The *power and influence network* was created by asking individuals who they believed were "powerful and influential" on the unit.
...
For the *get along with* network, individuals could have asymmetric nominations. That is, *i* could nominate *j* and *j* didn't necessarily nominate *i*. But, we are going to **symmetrize** the network by only taking ties for which ***both*** *i* and *j* indicated that they get along with the other person. This will give us an undirected network.
...
Wait, did you notice something about the unstandardized closeness scores? Go back and take a look. I will wait...
...
The mean distance is `r round( mean( ga_net_ucomp_distance ), 3 )`, meaning that the average node has to go through a lot of edges to reach other nodes. Again, a visual aid may help. Take a look at the plot below:
...
What is the fewest number of edges node 4 has to traverse to reach node 13? To reach 135? These are large distances! That is how we get such small closeness centrality scores.

## An Alternative

When dealing with disconnected graphs, restricting our analysis to the first component is one option. This works well when we have a single large component that captures most of the network. 
But, in some cases we might have multiple components and performing separate analyses can be cumbersome.

In such cases, we can use an alternate form of closeness developed by [Gil and Schmidt (1996)]() which the sum of the inverse distances to each node. 
In the `sna` package, this can be computed using the argument `cmode="suminvdir"` for directed networks and `cmode="suminvundir"` for undirected networks. 
For example, compare these two:

```{r}

# standardized closeness
closeness( ga_net_u, gmode="graph" ) 

closeness( ga_net_u, gmode="graph", cmode = "suminvundir" ) 
