...
# there are 11 vertices, 6 are bipartite (in the first mode), and 12 edges
...
Now that our object is created, we can take a look at a plot of the network using the `gplot()` function. Before we do so, let's take a look at a few changes we need to make. First, note that the `gplot()` function reads the labels by starting with the names in the first mode, then the names in the second mode. We can see this by printing the vertex labels that the function assigns with the `network.vertex.names()` function.
...
The density for this network is `r round( L / ( N * M ), 2 )`. *What is the interpretation of the density for this network?*
